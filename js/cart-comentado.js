...
// - Modales del carrito y checkout

document.addEventListener('DOMContentLoaded', () => {
  // ESTADO DEL CARRITO
  // Recupera el carrito desde localStorage o inicializa un array vacío
  // localStorage permite que el carrito persista entre sesiones del navegador
  let cart = JSON.parse(localStorage.getItem('aspenCart')) || [];
  
  // ELEMENTOS DEL DOM
  // Referencias a elementos HTML que necesitamos manipular
  const cartCountElement = document.querySelector('.cart-count'); // Contador de items en el ícono del carrito
  const cartItemsContainer = document.getElementById('cart-items'); // Contenedor de items en el modal del carrito
  const cartTotalElement = document.getElementById('cart-total'); // Elemento que muestra el total del carrito
  const checkoutItemsContainer = document.getElementById('checkout-items'); // Items en el modal de checkout
  const checkoutTotalElement = document.getElementById('checkout-total'); // Total en el modal de checkout
  
  // FUNCIONALIDAD DE PESTAÑAS
  // Maneja el cambio entre diferentes categorías de productos (Lift Tickets, Rentals, etc.)
  const tabButtons = document.querySelectorAll('.tab-btn'); // Botones de las pestañas
  const tabPanes = document.querySelectorAll('.tab-pane'); // Contenido de cada pestaña
  ...
  function removeFromCart(itemId) {
    ...
    if (itemIndex !== -1) {
      ...
      // Guarda los cambios y actualiza la UI
      saveCart();
      updateCartCount();
      renderCartItems();
    }
  ...
  // FUNCIÓN: Mostrar modal del carrito (sidebar)
  ...
  // FUNCIÓN: Mostrar modal de checkout
